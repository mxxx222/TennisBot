#!/usr/bin/env python3
"""
Generate Human-Readable Bet List for Football OU2.5
"""
...
def generate_bet_list(data):
    ...
    output = []
    output.append("⚽ FOOTBALL OU2.5 AI - BET LIST")
    output.append("="*80)
    ...
    for i, bet in enumerate(bets, 1):
        ...
        output.append("-" * 80)
        output.append(f"Match:      {match['home_team']} vs {match['away_team']}")
        output.append(f"League:     {match['league']}")
        home_goals = match.get('home_goals_avg', 0) or 0
        away_goals = match.get('away_goals_avg', 0) or 0
        total_avg = home_goals + away_goals
        if total_avg > 0:
            output.append(f"Avg Goals:  {total_avg:.1f} goals/match")
        output.append("")
        output.append(f"✅ RECOMMENDATION: {bet['recommended_bet']} 2.5")
...
if __name__ == '__main__':
    ...
    output_file = project_root / 'data' / 'football_ai' / 'bet_list.txt'
